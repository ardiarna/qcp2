...
session_start(); 
$akses = $_SESSION[$app_id]['app_priv']['55'];
?>
<style type="text/css">
    .adaborder {
        font-size:11px;
...
var frm = "include/ab_rpt_runhour.inc.php";

function lihatData(qar_id) {
    $('#myModal').modal('show');
    $.post(frm+"?mode=lihatdata", {qar_id:qar_id}, function(resp,stat){
...
$(document).ready(function () {
    ...
    if($("#frCari").valid()) {
        ...
        $("#dvLoading").show();
        $.post(frm+"?mode=urai&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val(), function(resp,stat){
            var o = JSON.parse(resp);
    ...
    $('#btnExport').click(function(){
        ...
        if (frmt == 'PDF') {
            ...
            window.open("include/mpdf-6.1.4/ab_rpt_runhour.pdf.php?tanggal="+$("#tglFrom").val()+"@"+$("#tglTo").val(),"",opsi);    
        } else if (frmt == 'XLSX') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xlsx&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val(),"",opsi);    
        } else if (frmt == 'XLS') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xls&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val(),"",opsi);    
        }
    });
});
</script>