            ...
            <div class="box-body" id="boxAwal">
                ...
                <form class="form-horizontal" id="frCari">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-2" style="margin-top:3px;">
                            <select class="form-control input-sm" id="fh_shift" name="fh_shift">
                        ...
                        <label class="col-sm-1 control-label" style="text-align:left;margin-top:3px;">Kiln :</label>
                        <div class="col-sm-2" style="margin-top:3px;">
                            <select class="form-control input-sm" id="fh_kiln" name="fh_kiln">
                                <option value="All">All</option>
                                <option value="1">1</option>
                                <option value="2">2</option>
                                <option value="3">3</option>
                ...
                <form class="form-horizontal" id="frExport" style="display:none;">
                    <div class="form-group">
                        <label class="col-sm-1 control-label" style="text-align:left;margin-top:3px;">No. Press :</label>
                        <div class="col-sm-2" style="margin-top:3px;">
...
var frm = "include/fg_firingcal_report.inc.php";

function lihatData(fh_id) {
    $('#myModal').modal('show');
    $.post(frm+"?mode=lihatdata", {fh_id:fh_id} , function(resp,stat){
...
$(document).ready(function () {
    ...
    $.post(frm+"?mode=cbosubplant", function(resp,stat){
        ...
        if($("#frCari").valid()) {
            ...
            $("#dvLoading").show();
            $.post(frm+"?mode=urai&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#fgf_sub_plant").val()+"&fh_shift="+$("#fh_shift").val()+"&fh_kiln="+$("#fh_kiln").val(), function(resp,stat){
                var o = JSON.parse(resp);
    ...
    $('#btnExport').click(function(){
        ...
        if (frmt == 'PDF') {
            ...
            window.open("include/mpdf-6.1.4/fg_firingcal_report.pdf.php?subplan="+$("#fgf_sub_plant").val()+"&tanggal="+$("#tglFrom").val()+"@"+$("#tglTo").val()+"&fh_shift="+$("#fh_shift").val()+"&fh_kiln="+$("#fh_kiln").val(),"",opsi);    
        } else if (frmt == 'XLSX') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xlsx&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#fgf_sub_plant").val()+"&fh_shift="+$("#fh_shift").val()+"&fh_kiln="+$("#fh_kiln").val(),"",opsi);    
        } else if (frmt == 'XLS') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xls&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#fgf_sub_plant").val()+"&fh_shift="+$("#fh_shift").val()+"&fh_kiln="+$("#fh_kiln").val(),"",opsi);    
        } 
    });
});
</script>