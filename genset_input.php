...
// print_r('<br/>'.json_encode($_SESSION[$app_id]['app_priv'],128).'<br/>');
...
<script type="text/javascript">
    var frm = "include/genset_input.inc.php";
    var vdropmenu = false;
    var validator = "";
    ...
    function simpanData(mode)
    {
        ...
        var rulenya = {
            ...
            qgh_no_urut: {required: true},
            qgh_genset: {required: true}
        };
        if (validator != "") {
            validator.destroy();
        }
            ...
            $.post(frm + "?mode=" + mode, $("#frEdit").serialize(), function (resp, stat) {
                if (resp == "OK") {
                    if (mode == "add") {
                        ...
                        swal("Data "+$("#qgh_id").val()+" berhasil diubah!", { icon: "info"});
                    }
                    // (mode == "add") ? alert("Data berhasil disimpan") : alert("Perubahan data "+$("#qgh_id").val()+" berhasil disimpan") ;
                    formAwal();
                    $.jgrid.gridUnload("#tblsm");
                    tampilTabel("#tblsm", "#pgrsm", $("#tglFrom").val() + "@" + $("#tglTo").val());
                } else {
    ...
    $(document).ready(function ()
    {
        ...
        $.post(frm + "?mode=cbogenset", function (resp, stat) {
            // $("#qgh_sub_plant").html(resp);
        });
    ...
    function addItems()
    {
        ...
        $("div.genset-row:last input:first").val('Genset-'+(baris.length+1));
        $("div.genset-row:last input:first").attr('name','qgh_no_urut['+(baris.length)+']');
        $("div.genset-row:last input:first").attr('id','qgh_no_urut'+(baris.length));
        $("div.genset-row:last input:last").val(0);
        $("div.genset-row:last input:last").attr('name','qgh_genset['+(baris.length)+']');
        $("div.genset-row:last input:last").attr('id','qgh_genset'+(baris.length));
    }

    function dropItems(x)
    {
        if ( $("div.genset-row").length > 3 ){
            $(x).closest($("div.genset-row")).remove()
        }else{
