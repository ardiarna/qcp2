...
function urai(){
	...
	if($count > 0) {
		foreach($qry as $ro){
			...
			if(!empty($ro['ic_ap_kabag_sts']) || !empty($ro['ic_ap_pm_sts'])){
				$btnEdit = '';
				$btnDel  = '';
			}
			...
			$responce->rows[$i]['cell']=array($ro['ic_id'],$ro['date'],$ro['ic_no_kendaraan'],$ro['ic_nm_material'],$ro['ic_lw'],$ro['ic_visco'],$ro['ic_kadar_air'],$ro['ic_hasil'],$ro['ic_apr'],$ro['kontrol']);
			$i++;
		}
	}
	echo json_encode($responce);
	...
	// if($d_dup['ic_id'] == '') {
		if($stat == "add") {
			...
			$sql = "INSERT INTO qc_ic_kebasahan_data(ic_id, ic_date, ic_no_kendaraan,
						ic_kd_material, ic_nm_material, ic_kadar_air, ic_keterangan, ic_rec_stat,
			 			ic_user_create, ic_date_create, ic_lw, ic_visco, ic_residu, ic_hasil)
					VALUES ('{$r[ic_id]}', '{$r[ic_date]}', '{$r[ic_no_kendaraan]}',
						'{$r[ic_kd_material]}', '{$r[ic_nm_material]}', '{$r[ic_kadar_air]}',
						'{$r[ic_keterangan]}', 'N', '{$r[ic_user_create]}', '{$r[ic_date_create]}', '{$r[ic_lw]}',
						'{$r[ic_visco]}', '{$r[ic_residu]}', '{$r[ic_hasil]}');";
			...
			$sql = "UPDATE qc_ic_kebasahan_data
					SET ic_date = '{$r[ic_date]}', ic_no_kendaraan = '{$r[ic_no_kendaraan]}',
						...
						ic_kadar_air = '{$r[ic_kadar_air]}',
						ic_keterangan = '{$r[ic_keterangan]}', ic_lw = '{$r[ic_lw]}', ic_visco = '{$r[ic_visco]}',
						ic_residu = '{$r[ic_residu]}', ic_hasil = '{$r[ic_hasil]}', ic_user_modify = '{$r[ic_user_modify]}',
						ic_date_modify = '{$r[ic_date_modify]}'
					WHERE ic_id = '{$r[ic_id]}';";
			$xsql = dbsave_plan($app_plan_id, $sql);
			if($xsql==="OK"){
				$sqlArm 	= "UPDATE qc_ic_kebasahan_data
											SET ic_date = '{$r[ic_date]}', ic_no_kendaraan = '{$r[ic_no_kendaraan]}',
												ic_kd_material = '{$r[ic_kd_material]}', ic_nm_material = '{$r[ic_nm_material]}',
												ic_kadar_air = '{$r[ic_kadar_air]}',
												ic_keterangan = '{$r[ic_keterangan]}', ic_lw = '{$r[ic_lw]}', ic_visco = '{$r[ic_visco]}',
												...
												ic_date_modify = '{$r[ic_date_modify]}'
											WHERE ic_id = '{$r[ic_id]}';";
				$xsql 	= dbsave_plan($app_armasi_id,$sqlArm);
			}
			$out = $xsql;
		}
		echo $out;
...
function detailtabel($stat) {
	...
	if($stat == 'edit' || $stat == 'view'){
		$sql  = "SELECT * FROM qc_ic_kebasahan_data WHERE ic_id = '{$_REQUEST[kode]}'";
