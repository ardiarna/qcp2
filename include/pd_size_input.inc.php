...
$akses = $_SESSION[$app_id]['app_priv']['129'];
...
function urai(){
	...
	if(!$sidx) $sidx = 1;
	$sql = "SELECT count(*) as count from qc_pd_size_header where op_rec_stat = 'N' and op_date >= '{$tglfrom}' and op_date <= '{$tglto}' $whsatu $whdua";
	...
	if($count > 0) { 
		...
		$sql = "SELECT * from qc_pd_size_header where op_rec_stat = 'N' and op_date >= '{$tglfrom}' and op_date <= '{$tglto}' $whsatu $whdua
...
function simpan($stat){
	...
	if($stat == "add") {
		...
		$sql = "SELECT max(op_id) as op_id_max from qc_pd_size_header where op_sub_plant = '{$r[op_sub_plant]}'";
		...
		$sql = "INSERT into qc_pd_size_header(op_sub_plant, op_id, op_date, op_shift, op_press, op_rec_stat, op_tekanan, op_format, op_ukuran, op_user_create, op_date_create) 
		...
		if($xsql == "OK") {
			...
			foreach ($r[op_mould] as $i => $value) {
				foreach ($r[op_value.$i] as $isi => $nilai) {
					$k2sql .= "INSERT into qc_pd_size_detail(op_id, op_mould, op_urut, op_value) 
		...
		$sql = "UPDATE qc_pd_size_header set op_user_modify = '{$r[op_user_modify]}', op_date_modify = '{$r[op_date_modify]}', op_tekanan = '{$r[op_tekanan]}', op_ukuran = '{$r[op_ukuran]}' where op_id = '{$r[op_id]}';";
		$xsql = dbsave_plan($app_plan_id, $sql); 
		if($xsql == "OK") {
			$k1sql = "DELETE from qc_pd_size_detail where op_id = '{$r[op_id]}';";
			...
			if($x1sql == "OK") {
				...
				foreach ($r[op_mould] as $i => $value) {
					foreach ($r[op_value.$i] as $isi => $nilai) {
						if(!empty($nilai)){
							$k2sql .= "INSERT into qc_pd_size_detail(op_id, op_mould, op_urut, op_value) values('{$r[op_id]}', '{$value}', '{$isi}', '{$nilai}');";
						}
					}
				}
				$out = dbsave_plan($app_plan_id, $k2sql);	
...
function hapus(){
	...
	$sql = "UPDATE qc_pd_size_header SET op_rec_stat = 'C' WHERE op_id = '{$op_id}';";
...
function detailtabel($stat) {
	...
	if($stat == "edit" || $stat == "view") {
		$op_id = $_POST['kode'];
		$sql0 = "SELECT * from qc_pd_size_header where op_id = '{$op_id}'";
		...
		$sql = "SELECT distinct op_mould as qpm_code from qc_pd_size_detail where op_id = '{$op_id}' order by op_mould";
	...
	foreach($qry as $r) {
		...
		if($stat == "edit" || $stat == "view") {
			$sql2 = "SELECT * FROM qc_pd_size_detail WHERE op_id = '{$op_id}' order by op_mould, op_urut";
			$responce->sql = $sql2; 
