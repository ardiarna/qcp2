...
function urai($excel = false){
	...
	$sql = "SELECT a.op_id, a.op_sub_plant  as subplan, a.op_date, a.op_shift, a.op_press, a.op_tekanan, a.op_format, a.op_ukuran, b.op_mould, b.op_urut, b.op_value 
			from qc.qc_pd_thickness_header a left join qc.qc_pd_thickness_detail b on a.op_id = b.op_id
	...
	if(is_array($arr_nilai)) {
		...
		$html .= '<div style="text-align:right;font-size:13px;font-weight:bold;">No : F.1002.QC.</div>
				  <div style="text-align:center;font-size:20px;font-weight:bold;">THICKNESS OUT PRESS</div>
...
function excel() {
	...
	header('Content-Disposition: attachment; filename="Thickness_out_press.xls"');
	echo urai(true);
}
...
function lihatdata(){
	global $app_plan_id;
	$op_id = $_POST['op_id'];
	$sql = "SELECT * from qc_pd_thickness_header where op_id = '{$op_id}'";
