					...
					if($grup == "01") {
						...
						$html .= '<div style="overflow-x:auto;"><table class="adaborder" '.$border.'>';
							...
							$sql1 = "SELECT a.qph_id, a.qph_date, a.qph_no_line, a.qph_shift as shift,  
											b.qpd_pd_seq as line, c.qpgd_control_desc as item_name, c.qpgd_standar as std, d.qgu_code as unit, 
							...
							if(is_array($qry1)) {
								...
								foreach($qry1 as $r1){
									...
									$arr_item["$r1[line]"] = $r1[item_name].'@'.$r1[unit].'@'.$r1[std];
								}
									...
									foreach($arr_item as $item => $itemVal1){
										...
										$html .= '<td class="text-center">'.$itemVal[2].'</td>';
										$html .= '<td class="text-center">'.$itemVal[1].'</td>';
							...
							if(is_array($qry2)) {
								...
								foreach($qry2 as $r2){
									...
									foreach($arr_item2 as $item2 => $itemVal21){
										...
										$html .= '<td></td>';
										$html .= '<td class="text-center">'.$itemVal2[1].'</td>';
							...
							if(is_array($qry3)) {
								...
								foreach($qry3 as $r3){
									...
									foreach($arr_item3 as $item3 => $itemVal31){
										...
										$html .= '<td></td>';
										$html .= '<td class="text-center">'.$itemVal3[1].'</td>';
										...
										foreach($arr_kolom3 as $tgl3 => $a_shift3){
											foreach($a_shift3[$press] as $shift3 => $a_mp3){
												foreach($a_mp3 as $mp3){
													...
													$html .= '<td class="text-right">'.$min3.'</td>';
													$html .= '<td class="text-right">'.$max3.'</td>';
...
function excel() {
	...
	header('Content-Disposition: attachment; filename="PRESS_DRYING('.$tanggal.').xls"');
	echo urai(true);
}
...
function lihatdata(){
	global $app_plan_id;
	$qph_id = $_POST['qsm_id'];
	$sql = "SELECT * from qc_pd_header WHERE qph_id = '{$qph_id}'";
	$rh = dbselect_plan($app_plan_id, $sql);
	$datetime = explode(' ',$rh[qph_date]);
	$rh[tgl] = cgx_dmy2ymd($datetime[0]);
	$rh[jam] = substr($datetime[1],0,5);
			
	$out = '<table class="table table-striped table-bordered table-condensed"><tr><td>ID : '.$rh[qph_id].'</td><td>Di-input Oleh : '.$rh[qph_user_create].'</td></tr><tr><td>Subplant : '.$rh[qph_sub_plant].'</td><td>Tanggal Input : '.$rh[qph_date_create].'</td></tr><tr><td>Tanggal : '.$rh[tgl].'</td><td>Di-edit Oleh : '.$rh[qph_user_modify].'</td></tr><tr><td>Jam : '.$rh[jam].'</td><td>Tanggal Edit : '.$rh[qph_date_modify].'</td></tr></table>';
	$responce->hasil=$out;
    echo json_encode($responce);
