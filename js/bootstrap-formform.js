	...
	return function(dom, fields) {
		...
		self.templates= {
			...
			input: _.template(
				...
				, {variable: 'data'}),
			date: _.template(
				'<div class="input-group date"><div class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></div><input type="text" name="<%= data.name %>" class="form-control" id="<%= data.id %>" <% if (data.value){ %>value="<%- data.value %>"<% } %>/></div>'
				, {variable: 'data'}),
			textarea: _.template(
