        ...
        <div class="box box-info">
            ...
            <div class="box-body" id="boxEdit" style="display: none;">
                <form class="form-horizontal" id="frEdit">
                    ...
                    <input type="hidden" name="pkey_qbm_plant_code" id="pkey_qbm_plant_code">
                    <input type="hidden" name="pkey_qbm_kode" id="pkey_qbm_kode">
                    <div class="form-group">
                        <label class="col-sm-2 control-label" style="text-align:left;">SUBPLANT</label>
                        <div class="col-sm-4" style="margin-top:3px;">
                            <select class="form-control input-sm" id="qbm_plant_code" name="qbm_plant_code"></select>   
                        </div>
                        <label class="col-sm-2 control-label" style="text-align:left;">NOMOR</label>
                        <div class="col-sm-4" style="margin-top:3px;">  
                            <input class="form-control input-sm" type="text" name="qbm_kode" id="qbm_kode" maxlength="2">
                        </div>    
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label" style="text-align:left;">KAPASITAS</label>
                        <div class="col-sm-4" style="margin-top:3px;">
                            <input type="text" class="form-control input-sm" id="qbm_capacity" name="qbm_capacity">
                        </div>
                        <label class="col-sm-2 control-label" style="text-align:left;">KETERANGAN</label>
                        <div class="col-sm-4" style="margin-top:3px;">
                            <input type="text" class="form-control input-sm" id="qbm_desc" name="qbm_desc">       
                        </div>
                    </div>
                    <div class="form-group">
...
var frm = "include/mdballmill.inc.php";
var vdropmenu = false;
var validator = "";
...
function tampilTabel(tblnya, pgrnya, pSubPlan = 'All') {
    ...
    jQuery(tblnya).jqGrid({
        ...
        datatype:"json",
        colModel:[
            {label:"PLANT", name:'qbm_plant_code', index:'qbm_plant_code', width:80, stype:'select', searchoptions:{value:":;A:A;B:B;C:C"}},
            {label:"NOMOR", name:'qbm_kode', index:'qbm_kode', width:80},
            {label:"KAPASITAS", name:'qbm_capacity', index:'qbm_capacity', width:90, sorttype:"int", formatter:'integer', align:'right'},
            {label:"KETERANGAN", name:'qbm_desc', index:'qbm_desc', width:400},
            {label:"KONTROL", name:'kontrol', index:'kontrol', width:50, align:'center'}
        ],
        sortname:'qbm_plant_code asc,qbm_kode', 
        sortorder:'asc', 
        styleUI:"Bootstrap",
...
function tambahData(){
    $.post(frm+"?mode=detailtabel", {stat:"add"}, function(resp,stat){
        ...
        $("#aded").val("add");
        $("#qbm_plant_code").html(o.sub_plan);
        $("#boxAwal").hide();
        $("#boxEdit").show(); 
    });
}

function editData(qbm_plant_code, qbm_kode){
    $.post(frm+"?mode=detailtabel", {stat:"edit",qbm_plant_code:qbm_plant_code,qbm_kode:qbm_kode}, function(resp,stat){
        var o = JSON.parse(resp);
        $("#aded").val("edit");
        $("#pkey_qbm_plant_code").val(o.qbm_plant_code);
        $("#pkey_qbm_kode").val(o.qbm_kode);
        $("#qbm_plant_code").html(o.sub_plan);
        $("#qbm_kode").val(o.qbm_kode);
        $("#qbm_capacity").val(o.qbm_capacity);
        $("#qbm_desc").val(o.qbm_desc);
        $("#boxAwal").hide();
        $("#boxEdit").show();  
    });
}

function hapusData(qbm_plant_code, qbm_kode){
    var r = confirm("Hapus data ball mill plant "+qbm_plant_code+" nomor "+qbm_kode+" ?");
    if (r == true) {
        $.post(frm+"?mode=hapus", {qbm_plant_code:qbm_plant_code,qbm_kode:qbm_kode}, function(resp,stat){
            if (resp=="OK") {
        ...
        $.post(frm+"?mode="+mode, $("#frEdit").serialize(), function(resp,stat){
          if (resp=="OK") {
            if (mode == "add") {
                ...
                alert("Perubahan data "+$("#qbm_kode").val()+" berhasil disimpan");
            }
            formAwal();
            $.jgrid.gridUnload("#tblsm");
            tampilTabel("#tblsm","#pgrsm");
...
$(document).ready(function (){
    ...
    $("#qbm_kode").afDigitOnly();
    $("#qbm_capacity").afNumericOnly();
    $("#qbm_desc").afInputVal();

    var rulenya = {
            qbm_plant_code:{required:true},
            qbm_kode:{required:true,maxlength:2},
            qbm_capacity:{required:true,number: true},
            qbm_desc:{required:true}
        };
