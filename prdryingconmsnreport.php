            ...
            <div class="box-body" id="boxAwal">
                ...
                <form class="form-horizontal" id="frCari">
                    <div class="form-group">
                        ...
                        <div class="col-sm-2" style="margin-top:3px;">
                            <select class="form-control input-sm" id="cmh_press" name="cmh_press">
                        ...
                        <label class="col-sm-1 control-label" style="text-align:left;margin-top:3px;"></label>
                        <div class="col-sm-2" style="margin-top:3px;">
...
var frm = "include/prdryingconmsnreport.inc.php";

function lihatData(cmh_id) {
    $('#myModal').modal('show');
    $.post(frm+"?mode=lihatdata", {cmh_id:cmh_id} , function(resp,stat){
...
$(document).ready(function () {
    ...
    $.post(frm+"?mode=cbosubplant", function(resp,stat){
        ...
        if($("#frCari").valid()) {
            ...
            $("#dvLoading").show();
            $.post(frm+"?mode=urai&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#cmbSubplan").val()+"&cmh_press="+$("#cmh_press").val(), function(resp,stat){
                var o = JSON.parse(resp);
    ...
    $('#btnExport').click(function(){
        ...
        if (frmt == 'PDF') {
            ...
            window.open("include/mpdf-6.1.4/prdryingconmsnreport.pdf.php?subplan="+$("#cmbSubplan").val()+"&tanggal="+$("#tglFrom").val()+"@"+$("#tglTo").val()+"&cmh_press="+$("#cmh_press").val(),"",opsi);    
        } else if (frmt == 'XLSX') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xlsx&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#cmbSubplan").val()+"&cmh_press="+$("#cmh_press").val(),"",opsi);    
        } else if (frmt == 'XLS') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xls&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#cmbSubplan").val()+"&cmh_press="+$("#cmh_press").val(),"",opsi);    
        } 
    });
});
</script>