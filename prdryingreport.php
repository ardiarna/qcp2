            ...
            <div class="box-body" id="boxAwal">
                ...
                <form class="form-horizontal" id="frCari">
                    ...
                    <div class="form-group">
                        ...
                        <label class="col-sm-2 control-label" style="text-align:left;margin-top:3px;">No. Line :</label>
                        <div class="col-sm-3" style="margin-top:3px;">
                            <select class="form-control input-sm" id="qph_no_line" name="qph_no_line">
...
var frm = "include/prdryingreport.inc.php";
...
$(document).ready(function () {
    ...
    // $("#qph_no_line").html("<option value='All'></option>");  
    $("#tglFrom").datepicker({
        autoclose:true,
        format:'dd-mm-yyyy',
        todayHighlight:true,
    ...
    $.post(frm+"?mode=cbosubplant", function(resp,stat){
        ...
        if($("#frCari").valid()) {
            ...
            $("#dvLoading").show();
            $.post(frm+"?mode=urai&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#fgf_sub_plant").val()+"&qph_no_line="+$("#qph_no_line").val(), function(resp,stat){
                var o = JSON.parse(resp);
    ...
    $('#btnExport').click(function(){
        ...
        if (frmt == 'PDF') {
            ...
            window.open("include/mpdf-6.1.4/prdryingreport.pdf.php?subplan="+$("#fgf_sub_plant").val()+"&tanggal="+$("#tglFrom").val()+"@"+$("#tglTo").val()+"&qph_no_line="+$("#qph_no_line").val(),"",opsi);    
        } else if (frmt == 'XLSX') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xlsx&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#fgf_sub_plant").val()+"&qph_no_line="+$("#qph_no_line").val(),"",opsi);    
        } else if (frmt == 'XLS') {
            opsi = "width=900,height=600,screenX=500,toolbars=1,location=0,directories=0,status=1,menubar=0,scrollbars=1,resizable='no'";
            window.open(frm+"?mode=excel&tipe=xls&tanggal=" +$("#tglFrom").val()+"@"+$("#tglTo").val()+"&subplan="+$("#fgf_sub_plant").val()+"&qph_no_line="+$("#qph_no_line").val(),"",opsi);    
        } 
    });
});
</script>